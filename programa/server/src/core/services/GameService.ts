...
export class GameService {
   ...
   * @param servidor - Gestor de partidas en memoria.
   * @param timerManager - Gestor de temporizadores.
   */
  constructor(
    private io: Server,
    private servidor: ServidorPartidas,
    private timerManager: TimerManager
  ) {
    this.timerManager.setSocketServer(io);
  }
  ...
  async handleMatchExpiration(matchId: string) {
    try {
      ...
      if (players.length >= 2) {
        ...
        console.log(
          `[GameService] Tiempo de espera agotado para partida ${matchId}. Iniciando con ${players.length} jugadores.`
        );
        ...
        console.log(
          `[GameService] Tiempo de espera agotado para partida ${matchId}. Eliminando por falta de jugadores.`
        );
  ...
  public async crearPartida(
    ...
    this.timerManager.startTimer(
      idPartida,
      config.TIEMPO_VIDA_PARTIDA_MIN * 60,
  ...
  public eliminarPartida(idPartida: string) {
    this.timerManager.clearTimer(idPartida);
  ...
  public prepararPartida(partidaId: string, requestedBySocketID?: string) {
    ...
    const tableroSerializado = partida.tablero.matriz.map((row: any[]) =>
      row.map((c: any) => ({ colorID: c.colorID, estado: c.estado }))
    );
    const gameConfig = {
      ...config,
  ...
  public iniciarPartida(partidaId: string, requestedBySocketID?: string) {
    ...
    this.timerManager.clearTimer(partidaId);
  ...
  public async activarMatch(partidaId: string, socketID: string) {
    ...
    if (!resultado.valido) {
      ...
      partida.tablero.matriz.forEach((row: any[]) =>
        row.forEach((c: any) => {
  ...
  public listarPartidasDisponibles() {
    return Array.from(this.servidor.partidasActivas.values())
      ...
      .map((p) => ({
        ...
        tiempoRestante: this.timerManager.getRemainingTime(p.idPartida),
  ...
  public getRemainingTime(partidaId: string): number {
    return this.timerManager.getRemainingTime(partidaId);
  }
}
