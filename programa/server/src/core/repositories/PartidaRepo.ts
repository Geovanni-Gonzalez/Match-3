...
export class PartidaRepo {
    ...
    static async marcarInicioPartida(codigoPartida: string) {
        ...
        try {
            await conn.execute(
                'UPDATE partida SET fecha_inicio = NOW() WHERE codigo_partida = ?',
                [codigoPartida]
            );
        } finally {
            conn.release();
     ...
     */
    static async guardarResultadosFinales(idPartidaCodigo: string, resultados: { idJugador: number, puntaje: number, esGanador: boolean, tiempoInvertido: number }[], idGanadorPrincipal: number | null) {
        const conn = await pool.getConnection();
        try {
            const [rows] = await conn.execute('SELECT id_partida FROM partida WHERE codigo_partida = ? LIMIT 1', [idPartidaCodigo]);
            if ((rows as any).length === 0) throw new Error('Partida no encontrada (codigo)');
            ...
            await conn.execute(
                'UPDATE partida SET fecha_fin = NOW(), id_ganador_principal = ? WHERE id_partida = ?',
                [idGanadorPrincipal, idPartida]
            );
