  ...
  private verificarExistenciaDeMatches(): boolean {
    ...
    for (let r = 0; r < R; r++) {
      for (let c = 0; c < C; c++) {
        ...
        // Horizontal
        if (c + 2 < C && this.matriz[r][c + 1].colorID === color && this.matriz[r][c + 2].colorID === color)
          return true;

        // Vertical
        if (r + 2 < R && this.matriz[r + 1][c].colorID === color && this.matriz[r + 2][c].colorID === color)
          return true;
        ...
        if (
          ...
          this.matriz[r + 2][c + 2].colorID === color
        )
          return true;
